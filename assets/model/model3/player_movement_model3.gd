...
# ======================== স্টেট মেশিন ========================
enum PlayerState {IDLE, WALK, RUN, JUMP, FALL, ATTACK, CROUCH, SLIDE, WALL_SLIDE, LEDGE_GRAB, CLIMB_UP}
var current_state: PlayerState = PlayerState.IDLE

# ======================== নোড রেফারেন্স ========================
@onready var head: Node3D = $head
@onready var player_mesh: Node3D = $CollisionShape3D/model3
@onready var anim_player: AnimationPlayer = $CollisionShape3D/model3/AnimationPlayer
...
func _update_stamina(delta: float) -> void:
	var is_exhausting = (current_state == PlayerState.RUN or current_state == PlayerState.ATTACK)
	if is_exhausting:
...
func _update_animation_state() -> void:
	if current_state in [PlayerState.ATTACK, PlayerState.SLIDE, PlayerState.LEDGE_GRAB, PlayerState.CLIMB_UP]:
		return
	if not is_on_floor():
		if velocity.y > 0.1:
			_change_state(PlayerState.JUMP)
